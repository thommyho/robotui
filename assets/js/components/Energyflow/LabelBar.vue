...
.label-bar-scale {
	border: 1px solid var(--robotui-gray);
	height: 14px;
	background: none;
	display: flex;
	...
	white-space: nowrap;
	border-radius: 0;
	transition: border-radius var(--robotui-transition-medium) linear;
}
...
.label-bar-icon {
	background-color: var(--robotui-background);
	transform: scale(1);
	color: var(--robotui-default-text);
	border-radius: 0;
	border: 0.25rem solid var(--robotui-background);
...
.label-bar--hide-icon .label-bar-icon {
	background-color: var(--robotui-default-text);
