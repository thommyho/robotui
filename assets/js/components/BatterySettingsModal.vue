...
.prioritySoc {
	...
	transition-duration: var(--robotui-transition-fast);
	opacity: 1;
}
...
.progress {
	...
	position: relative;
	border-radius: 10px;
	background-color: var(--robotui-box) !important;
}
.progress-bar {
	transition: height var(--robotui-transition-fast) linear;
}
.icon {
	transition: transform var(--robotui-transition-fast) linear;
	z-index: 1;
	border-radius: 0.5rem;
}
.custom-select {
