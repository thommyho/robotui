...
.progress {
	height: 100%;
	font-size: 1rem;
	background: var(--robotui-background);
}
...
.target-slider::-webkit-slider-thumb {
	...
	background-color: var(--robotui-gray);
	cursor: grab;
	border: none;
	opacity: 1;
	border-radius: var(--thumb-overlap);
	box-shadow: 0 0 6px var(--robotui-background);
	pointer-events: auto;
	transition: background-color var(--robotui-transition-fast) linear;
}
...
.target-slider--active::-webkit-slider-thumb {
	background-color: var(--robotui-dark-green);
}
...
.vehicle-target-soc--active {
	background-color: var(--robotui-box);
}
</style>
